...
class AuthorListViewTest(TestCase):
    ...
    def test_pagination_is_ten(self):
        response = self.client.get(reverse('authors'))
        # Once we have the response we query it for its status code
        self.assertEqual(response.status_code, 200)

        # Whether or not the response is paginated
        self.assertTrue('is_paginated' in response.context)
        # response.context, which is the context variable passed to the template by the view.
...
class RenewBookInstancesViewTest(TestCase):
    ...
    def test_redirect_if_not_logged_in(self):
        ...
        # Manually check redirect (Can't use assertRedirect, because the redirect URL in unpredictable)
...
class AuthorCreateViewTest(TestCase):
    ...
    def setUp(self):
        ...
        test_user2 = User.objects.create_user(username='test_user2', password='whatever')
    ...
    def test_forbidden_if_logged_in_but_not_correct_permission(self):
        login = self.client.login(username='test_user2', password='whatever')
        response = self.client.get(reverse('author-create'))
        self.assertEqual(response.status_code, 403)
    ...
    def test_date_of_death_initially_has_date_in_past(self):
        login = self.client.login(username='test_user', password='some_pasword')
        response = self.client.get(reverse('author-create'))
        self.assertEqual(response.status_code, 200)

        date_in_past = datetime.date.today() - datetime.timedelta(day=1)

        self.assertEqual(response.context['form'].initial['date_of_death'], date_in_past)

    def test_redirects_to_all_author_list_on_success(self):
        login = self.client.login(username='test_user', password='some_password')
        date_in_past = datetime.date.today() - datetime.timedelta(days=1)
        response = self.client.post(reverse('author-create'), {'date_of_death':date_in_past})
